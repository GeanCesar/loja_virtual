...
<body>
    ...
    <?php 
    	...
    	$dados = mysqli_query($conectar, "SELECT produto.id as IdP, carrinho.id as Id, produto.nome as NomeP, produto.preco as PrecoP, produto.foto as Foto FROM carrinho INNER JOIN produto on carrinho.Produto = produto.id WHERE dono = $id and carrinho.Ativo = 1 ORDER BY produto.id");	    
			...
			if($total > 0){
				...
				<table class='table'>
				    ...
				    <tbody>";
							...
							echo "<tr>
						        <td class='esquerda'>
						        	<img class='hidden-xs' src='fotos/"; echo $linha['Foto'];
						        	echo "'>
						        	<label>";
						        	echo $linha['NomeP'];
						        	echo "</label></td>						        
				...
				<label class='precoFinal'>PREÇO TOTAL: R$ ";
				echo number_format($preco,2); 
				echo "</label>
				<button class='botao-finalizar' onclick=\"finalizaCom()\">FINALIZAR COMPRA</button>
			</div>";
			}
			else{
				echo "<div class='centro'><h4 class='nulo'>Nenhum item no carrinho...<h4>";
    ...
    function sair(){
      ...
      function removeCar(id){
    	...
    	document.getElementById("conteudo").style.opacity = "0.3";
	  	...
	  	var htmlConfirm = "<div class='externa confirm' id='sai'><div class='col-md-4 col-xs-9 col-sm-6 col-lg-3 confirmar'> <div class='topoConfirm'><button class='fecho' onclick='fechaMessage()'>X</button> </div> <div class='conteudoConfirm'> <i class='fas fa-question iconeConfirm'></i> <h3 style='display:  block; margin-top: 10px;'>  Deseja remover do carrinho? </h3> </div> <div class='botoesConfirm'> <button class='botaoPrimario' onclick='removedoCarrinho("+id+")'>SIM</button> </a> <button class='botaoSecundario' onclick='fechaMessage()' >NÃO</button></div></div></div>";
      ...
      function finalizaCom(id){

    	document.getElementById("capa").style.opacity = "0.3";
    	document.getElementById("me").style.opacity = "0.3";
    	document.getElementById("conteudo").style.opacity = "0.3";
      	
	  	var htmlConfirm = "<div class='externa confirm' id='sai'><div class='col-md-4 col-xs-9 col-sm-6 col-lg-3 confirmar'> <div class='topoConfirm'><button class='fecho' onclick='fechaMessage()'>X</button> </div> <div class='conteudoConfirm'> <i class='fas fa-question iconeConfirm'></i> <h3 style='display:  block; margin-top: 10px;'>  Deseja finalizar a compra? </h3> </div> <div class='botoesConfirm'> <button class='botaoPrimario' onclick='Finaliza()'>SIM</button> </a> <button class='botaoSecundario' onclick='fechaMessage()' >NÃO</button></div></div></div>";
